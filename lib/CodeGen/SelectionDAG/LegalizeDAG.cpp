...
void SelectionDAGLegalize::LegalizeOp(SDNode *Node) {
  // FIXME: Beware of loads and stores generated by LegalizeOp. Pointer
  // operands are "illegal" in M6502. In that backend, all loads and stores
  // must be processed by LegalizeOperationTypes.
  // TODO: find a better solution to i16 pointers in M6502.
  DEBUG(dbgs() << "\nLegalizing: "; Node->dump(&DAG));

  if (Node->getOpcode() == ISD::TargetConstant ||
      Node->getOpcode() == ISD::TargetGlobalAddress ||
      Node->getOpcode() == ISD::TargetJumpTable ||
      Node->getOpcode() == ISD::TargetExternalSymbol) // Allow illegal target nodes.
    return;
